...
void main() {
  group('FluxerMediaUrl.userAvatar', () {
    test('returns null when hash is null or empty', () {
      expect(
        FluxerMediaUrl.userAvatar(userId: '1', hash: null),
        isNull,
      );
      expect(
        FluxerMediaUrl.userAvatar(userId: '1', hash: ''),
        isNull,
      );
    });
  ...
  group('FluxerMediaUrl.userBanner', () {
    test('returns null when hash is null', () {
      expect(
        FluxerMediaUrl.userBanner(userId: '1', hash: null),
        isNull,
      );
    });
  ...
  group('FluxerMediaUrl.guildIcon', () {
    test('returns null when hash is null', () {
      expect(
        FluxerMediaUrl.guildIcon(guildId: '10', hash: null),
        isNull,
      );
    });
    ...
    test('returns webp with animated=false for animated hash when animated is false', () {
      final String? actual = FluxerMediaUrl.guildIcon(
        guildId: '10',
        hash: 'a_icon',
      );
  ...
  group('FluxerMediaUrl.guildMemberMedia', () {
    ...
    test('returns gif for animated guild member avatar when animated is true', () {
      final String actual = FluxerMediaUrl.guildMemberMedia(
        guildId: '10',
        userId: '1',
        type: GuildMemberMediaType.avatar,
  ...
  group('FluxerMediaUrl.sticker', () {
    test('clamps size to 320 or 160', () {
      ...
      expect(
        FluxerMediaUrl.sticker(id: '1', size: 100),
        contains('size=160'),
      );
    });
